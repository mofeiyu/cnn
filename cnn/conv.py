...
def forward(A_prev, W, b, stride, pad):
    ...
    A_prev_pad = zero_pad(A_prev, pad)                                     # Select ith training example's padded activation
    ...
    for h in range(n_H):                   # loop over vertical axis of the output volume
        for w in range(n_W):               # loop over horizontal axis of the output volume
            for c in range(n_C):           # loop over the channels of the output volume
                ...
                dZ_tmp = dZ[:, h, w, c].reshape(m,1,1,1)
                dA_prev_pad[:,vert_start:vert_end, horiz_start:horiz_end, :] +=  W[:,:,:,c] * dZ_tmp
                dW[:,:,:,c] += np.sum(a_slice * dZ_tmp)
                db[c] += np.sum(dZ_tmp)
